...
assert.equal(s.activity,false,'Activation does not fabricate results');
const restoredWhilePaused=reconcile({...cart,integrations:{...cart.integrations,WhatsApp:true},employees:cart.employees.map(e=>e.id==='emma'?{...e,paused:true}:e)});
assert.equal(runtime(restoredWhilePaused,restoredWhilePaused.employees[1]).health,'Paused','Reconnection never overrides a manual pause');
const savedInbox=reconcile(JSON.parse(JSON.stringify({...initial(),drafts:{'olivia:reply':'Hello','ava:note':'Check policy'},readConversations:{ava:false},internalNotes:{ava:['Policy needs review']}})));
assert.equal(savedInbox.drafts['olivia:reply'],'Hello');assert.equal(savedInbox.drafts['ava:note'],'Check policy');assert.equal(savedInbox.internalNotes.ava[0],'Policy needs review');assert.equal(savedInbox.readConversations.ava,false);
console.log('Passed: Home entry, honest activation results, manual pause after reconnect, and Inbox state persistence.');
